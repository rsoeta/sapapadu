		...
		<div class="card p-4 shadow-sm border-0" style="border-radius:12px;">
			...
			<div class="row mb-4">
				<div class="col-md-4 col-4">
					<label class="small text-muted">Dusun</label>
					<select id="filterDusun" class="form-control"></select>
				</div>

				<div class="col-md-4 col-4">
					<label class="small text-muted">RW</label>
					<select id="filterRw" class="form-control"></select>
				</div>

				<div class="col-md-4 col-4">
					<label class="small text-muted">RT</label>
			...
			<div class="row mb-4">
				<div class="col-md-6 text-center">
					<h6 class="text-muted mb-2">Komposisi Pembayaran</h6>
					<canvas id="chartKomposisi" style="max-height:280px;"></canvas>
				</div>

				<div class="col-md-6 text-center">
					<h6 class="text-muted mb-2">Timeline Setoran</h6>
					<canvas id="chartTimeline" style="max-height:280px;"></canvas>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-md-6">
					<div class="card border-0 shadow-sm">
						...
						<ul id="topList" class="list-group list-group-flush small"></ul>
					</div>
				</div>
				...
				<div class="col-md-6">
					<div class="card border-0 shadow-sm">
						...
						<ul id="bottomList" class="list-group list-group-flush small"></ul>
					</div>
				</div>
			</div>
			...
			<div class="footer">
				...
				<div class="copyright">
					<div class="container">
						<div class="row">
							<div class="col-md-12">
								<p>Copyright &copy; 2022 - <?= date('Y'); ?> All Right Reserved By Pemerintah Desa <?= ucwords(strtolower(infoApp()->nama_desa)); ?> Kec. <?= ucwords(strtolower(infoApp()->nama_kecamatan)); ?></p>
							</div>
						</div>
					</div>
				</div>
	...
	<script>
		...
		async function loadKomposisi() {
			...
			const res = await fetch(
				`/dashboard/komposisi?tahun=${DEFAULT_TAHUN}&dusun=${dusun}&rw=${rw}&rt=${rt}`
			);
		...
		async function loadRanking() {
			...
			data.slice(0, 10).forEach(r => {
				...
				top += `<li class="list-group-item">
            RT ${r.rt_fix}/RW ${r.rw_fix}
            <br><small>${r.alamat_rt||''}</small>
            <span style="float:right;color:${color}">${p.toFixed(2)}%</span>
        </li>`;
			});
			...
			data.slice(-10).reverse().forEach(r => {
				...
				bottom += `<li class="list-group-item">
            RT ${r.rt_fix}/RW ${r.rw_fix}
            <br><small>${r.alamat_rt||''}</small>
            <span style="float:right;color:${color}">${p.toFixed(2)}%</span>
        </li>`;
