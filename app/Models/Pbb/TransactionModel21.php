...
class TransactionModel21 extends Model
{
    protected $table      = 'pbb_transaksi21';
    ...
    public function search($keyword)
    {
        ...
        return $this->table('pbb_transaksi21')->like('tr_faktur', $keyword)
    ...
    {
        $query = $this->db->query("SELECT * FROM pbb_transaksi21 a
    ...
    {
        $builder = $this->db->table('pbb_transaksi21');
        $builder->select('*');
        $builder->join('pbb_pelanggan', 'pbb_pelanggan.id_pelanggan = pbb_transaksi21.pelanggan_id');
        $query = $builder->get();
    ...
    public function getDetail($id_tr = false)
    {
        ...
        $builder->select('*');
        $builder->join('pbb_pelanggan', 'pbb_pelanggan.id_pelanggan = pbb_transaksi21.pelanggan_id');
        $builder->join('pbb_detailtrans21', 'pbb_detailtrans21.dettr_faktur = pbb_transaksi21.tr_faktur');
        $builder->where(['pbb_transaksi21.id_tr' => $id_tr]);
