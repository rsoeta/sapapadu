...
class Pages extends BaseController
{
    ...
    //     $totalTarget = array_sum(array_column($setoranPerDusun, 'dataTarget'));
    //     $totalCapaian = array_sum(array_column($setoranPerDusun, 'dataCapaian'));
    //     $totalPersen = $totalTarget > 0 ? ($totalCapaian / $totalTarget) * 100 : 0;
    ...
    public function index2()
    {
        ...
        // FORMAT DATA + SISA
        foreach ($setoranPerDusun as $key => $row) {
            $setoranPerDusun[$key]->dusun = sprintf('%03d', $row->dusunNama);
            $setoranPerDusun[$key]->dataSisaPersentase = 100 - $row->dataPersentase;
        }

        foreach ($setoranPerRw as $key => $row) {
            $setoranPerRw[$key]->rw = sprintf('%03d', $row->rwNama);
            $setoranPerRw[$key]->dataSisaPersentase = 100 - $row->dataPersentase;
        }

        foreach ($setoranPerRt as $key => $row) {
            $setoranPerRt[$key]->rt = sprintf('%03d', $row->rtNama);
            $setoranPerRt[$key]->dataSisaPersentase = 100 - $row->dataPersentase;
        }
        ...
        // SORT RANKING (RT)
        usort($setoranPerRt, function ($a, $b) {
        ...
        return view('pbb/pages/dashboard_v2', [
