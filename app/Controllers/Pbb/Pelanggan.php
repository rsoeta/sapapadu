...
class Pelanggan extends BaseController
{
    ...
    public function ambildata()
    {
        if ($this->request->isAJAX()) {
            ...
            try {
                ...
                $msg = [
                    'data' => view('pbb/pelanggan/pelanggan', [
                        'tampildata' => $data
                    ])
                ];
    ...
    public function formmodal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('pbb/pelanggan/modal_add')
            ];
            ...
            if (!$doValid) {
                ...
                $msg = [
                    'error' => [
                        'errorNik' => $validation->getError('nik'),
                        'errorNamapelanggan' => $validation->getError('nama_pelanggan'),
                        'errorNoHp' => $validation->getError('no_hp'),
                        'errorAlamat' => $validation->getError('alamat_pelanggan'),
                        'errorDusun' => $validation->getError('dusun'),
                ...
                $data = [
                    ...
                    'id_rw' => $this->request->getVar('rw'),
                    'id_rt' => $this->request->getVar('rt'),
                ];

                $this->pelanggan->insert($data);
                $insertId = $this->pelanggan->insertID();

                $msg = [
                    'sukses' => 'Data Pelanggan berhasil disimpan',
    ...
    public function formedit()
    {
        ...
        if ($this->request->isAJAX()) {
            ...
            return $this->response->setJSON([
                'sukses' => view('pbb/pelanggan/modal_edit', $data)
            ]);
        }
    }
